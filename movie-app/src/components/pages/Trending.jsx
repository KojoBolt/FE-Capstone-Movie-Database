...
const genres = {
  trending: "https://api.themoviedb.org/3/discover/movie?api_key=b3ef0caea1e5686a7ca35397f079a099",
};
...
const Trending = () => {
  ...
  const fetchFav = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${genres.trending}&page=${currentPage}`);
    ...
    <div className="bg-black overflow-scroll h-screen w-full">
      ...
      <div className={`${OpenSideBar ? "ml-[400px]" : "ml-0"}`}>
        {currentMovies.map((movie) => (
          <div key={movie.id} className="flex flex-wrap gap-6 p-8 transition-all duration-300">
            <img
